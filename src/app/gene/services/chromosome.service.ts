...
// api
import { ChromosomeGetReply, ChromosomeGetRequest }
  from '@gcv-assets/js/grpc/gcv/services/chromosome_pb';
import { ChromosomeClient }
  from '@gcv-assets/js/grpc/gcv/services/ChromosomeServiceClientPb';
...
export class ChromosomeService extends HttpService {
  ...
  Observable<Track> {
    ...
    if (request.type === GET || request.type === POST) {
      ...
      const client = new ChromosomeClient(request.url);
