...
import { MicroSyntenySearchReply, MicroSyntenySearchRequest }
  from '@gcv-assets/js/grpc/gcv/services/microsyntenysearch_pb';
import { MicroSyntenySearchClient }
  from '@gcv-assets/js/grpc/gcv/services/MicrosyntenysearchServiceClientPb';
...
export class MicroTracksService extends HttpService {
  ...
  Observable<Track[]> {
    ...
    if (request.type === GET || request.type === POST) {
      ...
      const client = new MicroSyntenySearchClient(request.url);
