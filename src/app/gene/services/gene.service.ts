...
// api
import { GenesGetReply, GenesGetRequest }
  from '@gcv-assets/js/grpc/gcv/services/genes_pb';
import { GenesClient }
  from '@gcv-assets/js/grpc/gcv/services/GenesServiceClientPb';
...
export class GeneService extends HttpService {
  ...
  getGenes(genes: string[], serverID: string): Observable<Gene[]> {
    ...
    if (request.type === GET || request.type === POST) {
      ...
      const client = new GenesClient(request.url);
